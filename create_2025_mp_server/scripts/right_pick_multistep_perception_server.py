...
from create_2025_mp_server_msgs.msg import PoseAction, PoseActionGoal, PoseActionResult
import actionlib
...
class Motion_planner:
    ...
    def __init__(self) -> None:
        ...
        # create action server for pick
        self.pick_server = actionlib.SimpleActionServer(
            "right_pick", PoseAction, self.pick_callback, auto_start=False
        )
    ...
    def pick_callback(self, goal:PoseActionGoal):
        rospy.loginfo("Received pick")
        start = goal.pose
        prompt = goal.prompt
        
        success = self.pick(start, prompt)
        
        # set goal to success
        result = PoseActionResult()
        result.result = success

        if success:
            self.pick_server.set_succeeded(result)
        else:
            self.pick_server.set_aborted(result)


    def pick(self,start:PoseStamped, prompt:String):
        rospy.loginfo("Started pick with start : %s", start)
...
if __name__  == "__main__":
    rospy.init_node("right_pick_server", anonymous=True)
    mp = Motion_planner()
    rospy.spin()
